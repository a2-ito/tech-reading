...
author: "Jessica Apotheker, Sylvain Duranton, Vladimir Lukic, Nicolas de Bellefonds, Sesh Iyer, Olivier Bouffault, Romain de Laubier (BCG)"
...
BCG が 1,800 人超の経営層に実施した調査 (BCG AI Radar) の第 2 回レポート。
AI は経営課題として定着し、4 分の 3 の経営層が 2025 年の最重要課題の 3 本に挙げている。
...
成果を出した企業とそれ以外を分けたのは、投資額ではなく対象の絞り込みだった。
用例の数は 3.5 件対 6.1 件で、少ない側が 2.1 倍の ROI を見込んでいる。
「あれもこれも」と手を広げる方が前進している感覚は得られるが、数字は逆を示している。
加えて、成果を出した企業は投資先が小規模な効率化ではなく基幹業務そのものの作り替えに寄っている。
...
### 2. 10-20-70 — 難所はアルゴリズムではない

労力の配分をアルゴリズム 1 割、データと技術 2 割、人・プロセス・文化の変革 7 割にせよ、という原則。
エンジニアリング側の感覚だと投資はモデルとデータに向きがちだが、この調査では組織側が主戦場だと位置づけられている。
「ソフトな部分こそが難所だった」という一文が、この報告の要約になっている。
...
過半数の企業が AI 施策の財務 KPI を追跡していない。
逆に価値を出せた 4 分の 1 は、少数の施策に絞ったうえで運用指標と財務指標の両方を体系的に測っている。
効果測定の欠如は impact gap の原因であると同時に、gap の存在に気づけない理由でもある。
...
従業員の 4 分の 1 に AI 研修を実施できた企業は 3 分の 1 未満。
興味深いのは、この遅れを「雇用を脅かす技術だからこそ、従業員が安心して使えるところまで育成が要る」という文脈で語っている点だ。
実際、3 分の 2 の企業は人員規模を維持する見込みだと答えている。
育成は生産性の話であると同時に、納得感を作る施策として位置づけられている。
